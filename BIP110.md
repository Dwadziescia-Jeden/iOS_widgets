...
// Track when script started for "updated X min ago" display
...
async function createWidget() {
  ...
  // Helper: add centered text row inside a vertical stack
  ...
  addCenteredText(leftCol, signalingPct + "%", Font.boldSystemFont(32), new Color("#FFFFFF"));
  ...
  let threshold = w.addText("55% threshold required");
  threshold.centerAlignText();
  threshold.font = Font.systemFont(9);
  threshold.textColor = new Color("#555555");
  
  // Footer - last updated, shown as relative time (auto-updates on screen)
  let updatedRow = w.addStack();
  updatedRow.layoutHorizontally();
  updatedRow.addSpacer();
  let updatedLabel = updatedRow.addText("Updated: ");
  updatedLabel.font = Font.systemFont(9);
  updatedLabel.textColor = new Color("#555555");
  let updatedTime = updatedRow.addDate(scriptStart);
  updatedTime.applyRelativeStyle();
  updatedTime.font = Font.systemFont(9);
  updatedTime.textColor = new Color("#555555");
  updatedRow.addSpacer();
